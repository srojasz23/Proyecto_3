...
USE SistemaAsistenciaDB2;
GO


INSERT INTO Ciclos (Codigo, Descripcion, FechaInicio, FechaFin, IsActive)
VALUES 
('2025-1', 'Primer ciclo académico 2025', '2025-03-01', '2025-07-31', 1),
('2025-2', 'Segundo ciclo académico 2025', '2025-08-01', '2025-12-15', 1);
GO
...
INSERT INTO Cursos (Nombre, Descripcion, Creditos)
VALUES
('Programación I', 'Fundamentos de C#', 4),
('Programación II', 'POO en C#', 4),
('Base de Datos I', 'Modelado y SQL', 3),
('Base de Datos II', 'Procedimientos y Transacciones', 3),
('Redes I', 'Conceptos de redes y protocolos', 3),
('Sistemas Operativos', 'Arquitectura y procesos', 3),
('Ingeniería de Software I', 'Metodologías de desarrollo', 4),
('Matemática Discreta', 'Lógica y conjuntos', 3),
('Estadística', 'Probabilidad y análisis de datos', 3),
('Ética Profesional', 'Valores y responsabilidad', 2);
GO
...
BEGIN
    ...
    VALUES (
        CONCAT('docente', @i + 1),
        0x5994471ABB01112AFCC18159F6CC74B4F511B99806DA59B3CAF5A9C173CACFC5,
        CONCAT('Docente ', @i, ' Prueba'),
        CONCAT('docente', @i, '@correo.com'),
        2, 1
    );
    SET @i += 1;
END;
...
BEGIN
    ...
    VALUES (
        CONCAT('alumno', @j + 1),
        0x5994471ABB01112AFCC18159F6CC74B4F511B99806DA59B3CAF5A9C173CACFC5,
        CONCAT('Alumno ', @j, ' Pérez'),
        CONCAT('alumno', @j, '@correo.com'),
        3, 1
    );

    INSERT INTO Alumnos (UserId, NombreCompleto, Seccion, CicloCodigo)
    SELECT UserId, CONCAT('Alumno ', @j, ' Pérez'), 
           CASE WHEN @j <= 25 THEN 'A' ELSE 'B' END, 
           CASE WHEN @j <= 25 THEN '2025-1' ELSE '2025-2' END
    FROM Usuarios WHERE Username = CONCAT('alumno', @j + 1);
...
SELECT 
    TOP 10 c.CursoId,
    d.DocenteId,
    CASE WHEN d.DocenteId <= 2 THEN 1 ELSE 2 END,
    CASE WHEN d.DocenteId <= 8 THEN 1 ELSE 2 END,
    CASE WHEN d.DocenteId % 2 = 0 THEN 'A' ELSE 'B' END
FROM Cursos c
CROSS JOIN Docentes d
WHERE d.DocenteId <= 10;
GO


SELECT COUNT(*) AS TotalAlumnos FROM Alumnos;
SELECT COUNT(*) AS TotalDocentes FROM Docentes;
SELECT COUNT(*) AS TotalCursos FROM Cursos;
SELECT COUNT(*) AS TotalCiclos FROM Ciclos;
SELECT COUNT(*) AS TotalAulas FROM Aulas;
GO
...
WITH Picks AS (
    SELECT 
        a.AlumnoId,
        s.AsignacionId,
        ROW_NUMBER() OVER (PARTITION BY a.AlumnoId ORDER BY NEWID()) AS rn
    FROM Alumnos a
    CROSS JOIN Asignaciones s
)
INSERT INTO Inscripciones (AlumnoId, AsignacionId, FechaRegistro)
SELECT p.AlumnoId, p.AsignacionId, GETDATE()
FROM Picks p
LEFT JOIN Inscripciones i 
       ON i.AlumnoId = p.AlumnoId AND i.AsignacionId = p.AsignacionId
WHERE p.rn <= 3
  AND i.InscripcionId IS NULL;
GO


INSERT INTO Notas (AlumnoId, AsignacionId, PC1, PC2, Parcial, Final)
SELECT 
    i.AlumnoId,
    i.AsignacionId,
    ROUND((ABS(CHECKSUM(NEWID())) % 201) / 10.0, 2) AS PC1,
    ROUND((ABS(CHECKSUM(NEWID())) % 201) / 10.0, 2) AS PC2,
    ROUND((ABS(CHECKSUM(NEWID())) % 201) / 10.0, 2) AS Parcial,
    ROUND((ABS(CHECKSUM(NEWID())) % 201) / 10.0, 2) AS Final
FROM Inscripciones i
LEFT JOIN Notas n 
       ON n.AlumnoId = i.AlumnoId AND n.AsignacionId = i.AsignacionId
WHERE n.NotaId IS NULL;
GO

SELECT COUNT(*) AS InscripcionesTotales FROM Inscripciones;
SELECT COUNT(*) AS NotasTotales FROM Notas;


SELECT TOP 20 
    n.NotaId, n.AlumnoId, n.AsignacionId, n.PC1, n.PC2, n.Parcial, n.Final, n.Promedio
FROM Notas n
